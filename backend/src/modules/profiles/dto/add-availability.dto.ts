...
import { IsBoolean, IsDateString, IsInt, IsNotEmpty, IsOptional, IsString, Matches, Max, Min } from 'class-validator';
...
export class AddAvailabilityDto {
  ...
  @ApiProperty({ example: '18:00', description: 'Start time in 24-hour format (HH:mm)' })
  @IsString()
  @IsNotEmpty({ message: 'Start time is required' })
  @Matches(/^([01]\d|2[0-3]):([0-5]\d)$/, { message: 'Start time must be formatted as HH:mm' })
  startTime: string;

  @ApiProperty({ example: '21:00', description: 'End time in 24-hour format (HH:mm)' })
  @IsString()
